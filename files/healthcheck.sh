...
retval=$(
	...
	for domain in anyqast-dnstest.com; do
		for test in 'A 0.0.0.0' 'AAAA ::' 'TXT "'"${domain}"'"' 'MX 0 '"${domain}"'.'; do
			read qtype expected <<<$(echo "${test}")
			result=$(dig -p 2053 +short +tries=1 +timeout=1 +noall +answer "@${healthcheck_ip}" -c IN -q "$(cat /dev/urandom | head -c 512 | sha512sum | head -c 16).${domain}." -t "${qtype}" 2>&1)
